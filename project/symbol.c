...
struct symbol *add_symbol(	struct table *tablep, 
							char *name, 
							unsigned char kind,
							unsigned char type, 
							unsigned int size, 
...
struct symbol *lookup_symbol (char *name)
{
	...
	if ( (symbolp = find_symbol (current_table, name) ) == NULL )
	{
...
void print_table ( struct table *tablep)
{
	...
	for ( i = 0; i < HASH_SIZE; i++)
	{
		...
		{
			fprintf(stdout, "%-10s %-10s %-10s %-10s %10d %10d\n", 
					tablep->name, 
					symbolp->name, 
					symbol_kind_list[symbolp->kind],
					symbol_type_list[symbolp->type], 
					symbolp->size, 
					symbolp->offset);
		}
	}
